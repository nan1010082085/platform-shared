...
// ────────────────────────────────────────────
// 本地最小类型（避免 platform-shared 依赖 flow-shared）
// ────────────────────────────────────────────
...
export interface RuntimeFlowInstanceData {
  id: string
  definitionId: string
  status: string
  startedAt: Date
...
export type RuntimeAIRequest =
  | { type: 'recommend-assignee'; task: RuntimeTaskData; context: ExecutionContext }
  | { type: 'evaluate-condition'; expression: string; variables: Record<string, unknown> }
  | { type: 'predict-outcome'; task: RuntimeTaskData; formData?: Record<string, unknown> }
  | { type: 'detect-anomaly'; instance: RuntimeFlowInstanceData; tasks: RuntimeTaskData[] }
...
export class RuntimeAgent {
   ...
   */
  async detectAnomaly(
    instance: RuntimeFlowInstanceData,
    tasks: RuntimeTaskData[],
